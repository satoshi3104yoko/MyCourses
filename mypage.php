	...
	while($row = $stmt->fetch(PDO::FETCH_ASSOC)){
		if(empty($day_period[$row["day"]][$row["period"]])){
				...
				$color[$row["day"]][$row["period"]] = $row["color"];
						...
						$day_period[$row["day"]][$row["period"]] = $row["course"]."<br>".$row["instructor"]."<br>".$row["room"];
						$color[$row["day"]][$row["period"]] = $row["color"];
	  	}
	  }
	}
...
<div class="cp_cont">
    ...
    <div class="page_top">
			<h1 class="mypage_tt_name"><?php if(!empty($tt_name)): ?>【<?=$tt_name?>】<?php endif; ?></h1>
   	</div>
		<div class="mypage_head">
		<p class="mypage_edit1">
			<a href="account_edit_form.php">ユーザー情報編集</a>
		</p>
		<p class="mypage_edit2">
			<a href="tt_edit.php">時間割・科目の追加・編集</a>
		</p>
		<form action="mypage.php" method="post" class="form">
			<select name="tt_name" class="mypage_tt_select">
				<?=$tt_select?>
			</select>
			<input type="submit" name="tt_select" value="表示" class="mypage_tt_btn"><br>
		</form>
	  </div>
		<div class="tt">
			<table border="1">
